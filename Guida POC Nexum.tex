...
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}X@{}}
	...
	{Verificatore} & Alberto Pignat \\
	{Redattori} & Alberto Autiero, Marco Piro, Marco Favero, Luca Slongo \\
...
1.0.0 & 31/01/2026 & Approvazione del documento & - & - & Luca Slongo \\
\hline
0.1.0 & 31/01/2026 & Verifica del documento & - & Alberto Pignat & - \\
\hline
...
\begin{itemize}
    \item \textbf{macOS}:
    \begin{itemize}
        \item[-] \textit{Se Homebrew non è installato, eseguire prima:} \\
        \texttt{\footnotesize /bin/bash -c "\$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"}
        \item[-] Installazione Ruby: \\
        \texttt{brew install ruby}
    \end{itemize}
    
    \item \textbf{Linux (Ubuntu/Debian)}: \\
    \texttt{sudo apt-get install ruby-full build-essential}
    
\end{itemize}

\noindent Dopo l'installazione, \textbf{per macOS}, è necessario configurare il percorso:

\begin{lstlisting}[language=bash, caption=Configurazione PATH Ruby su macOS]
echo 'export PATH="/opt/homebrew/opt/ruby/bin:$PATH"' >> ~/.zshrc
source ~/.zshrc
\end{lstlisting}

\noindent \textbf{Per Ubuntu/Debian} invece:

\begin{lstlisting}[language=bash, caption=Configurazione PATH Ruby su Ubuntu/Debian]
echo 'export PATH="$(ruby -e "print Gem.user_dir")/bin:$PATH"' >> ~/.bashrc
source ~/.bashrc
\end{lstlisting}

\noindent Successivamente, verificare l'installazione:
...
\textbf{macOS}:
\begin{lstlisting}[language=bash, caption=Installazione e configurazione PostgreSQL 18 su macOS]
...
\subsubsection{Creare il file .env e installare le dipendenze Rails}
...
\textbf{macOS}:
\begin{lstlisting}[language=bash, caption=Creare .env su macOS]
cd backend
bundle install
cp .env.example .env
\end{lstlisting}
...
\textbf{Ubuntu/Debian}:
\begin{lstlisting}[language=bash, caption=Creare .env e installare le dipendenze Rails su Ubuntu/Debian]
cd backend
bundle install
cp .env.example .env
\end{lstlisting}

\noindent In caso in cui bundle install restituisca errori di librerie mancanti, eseguire:
\begin{lstlisting}[language=bash, caption=Librerie mancanti su Ubuntu/Debian]
sudo apt-get install -y libyaml-dev pkg-config
\end{lstlisting}

\noindent In caso in cui bundle install restituisca errori di installazione delle gemme, eseguire:
\begin{lstlisting}[language=bash, caption=Correzione installazione di gemme su Ubuntu/Debian]
bundle config set --local path 'vendor/bundle'
bundle config set --local cache_path "$HOME/.bundle/cache"
...
\begin{itemize}
    \item \textbf{Windows}: Esegui PowerShell o il prompt dei comandi come amministratore.
    \item \textbf{macOS/Linux}: Evita di usare \texttt{sudo} con \texttt{brew} o \texttt{npm} per non corrompere i permessi. Se ricevi errori come "Permission denied" o "EACCES", riprendi il possesso della cartella corrente eseguendo: \\
    \texttt{sudo chown -R \$USER .}
\end{itemize}
...
\begin{lstlisting}[language=bash, caption=Inizializzazione manuale Database]
...
\begin{itemize}
    \item Verifica che la password nel file \texttt{backend/.env} corrisponda a quella dell'utente \texttt{postgres} impostata durante l'installazione.
    \item \textbf{Windows}: Usa lo script \texttt{verifica-postgres.ps1} per testare la connessione.
    \item \textbf{macOS/Linux}: Per testare la connessione manualmente, apri il terminale e lancia:
    \begin{lstlisting}[language=bash]
    psql -h localhost -U postgres
    \end{lstlisting}
    Se compare il prompt \texttt{postgres=\#}, la connessione è riuscita (digita \texttt{\textbackslash q} per uscire).
    Se richiede una password, prova a inserirne una vuota o quella che hai messo nel file \texttt{.env}.
\end{itemize}
...
\begin{lstlisting}[language=bash, caption=Installazione dipendenze mancanti]
cd backend
bundle install
\end{lstlisting}
...
Per l'utente corrente eseguire il seguente comando:

% Cambiato da 'powershell' a 'bash' per evitare errori di compilazione
\begin{lstlisting}[language=bash, caption=Cambiare policy di esecuzione PowerShell]
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser
\end{lstlisting}

\noindent Questo comando permette l'esecuzione di script locali e richiede la firma solo per script scaricati da Internet.

\vspace{0.2cm}
\noindent \textbf{Nota}: Potrebbe essere richiesto di confermare digitando \texttt{S} (Sì) o \texttt{Y} (Yes).
